...
function DownloadControl(){
  ...
  this.node.find('svg.download').on('click',$.proxy(function(editor,event){
    ...
    editor.downloadLevelEditorData();
  },null,this));
  ...
  this.downloadLevelEditorData=function(){
    ...
    var editorControl = this.node.parent().data('node');
    var data = editorControl.getData();

    //check for name
    if(data.name !== undefined && data.name !== ''){
      this.saveFile(data);
    }else{
      editorControl.nameControl.error();
    }
  };
